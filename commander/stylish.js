    ...
    const result = data.map((item) => {
        if (item.keyStatus === "object") {
            ...
            if (item.keyStatus === "removed" || item.keyStatus === "object, removed") {
                return `${sign.repeat(int + 2)}- ${item.keyName}: ${item.keyValue} # Удалено`
            }
            ...
            if (item.keyStatus === "changed") {
                return [
                    `${sign.repeat(int + 2)}- ${item.keyName}: ${item.keyValue} # Строе значение`,
                    `${sign.repeat(int + 2)}+ ${item.keyName}: ${item.keyValue2} # Новое значение`
                ].join('\n')

            }
            if (item.keyStatus === "added" || item.keyStatus === "object, add") {
                return `${sign.repeat(int + 2)}+ ${item.keyName}: ${item.keyValue} # Добавлено`
            }
        }
